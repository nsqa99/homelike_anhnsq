class AuthenticationService
  ...
  def validate_refresh_token(token, type)
    token_owner_type = TYPE_MAPPING[type]
    
    refresh_token = token_owner_type[:refresh_token].find_by(token: token)
    owner_id = refresh_token.try(token_owner_type[:id])
    token_owner = token_owner_type[:klass].find_by(id: owner_id)

    raise JwtService::VerificationError unless refresh_token
    
    if refresh_token.status_active?
      payload = payload || owner_id
      data = JwtService.decode(token)
  ...
  def generate_tokens(owner_id, payload, type, expire_refresh_time = DEFAULT_REFRESH_EXPIRE_TIME)
